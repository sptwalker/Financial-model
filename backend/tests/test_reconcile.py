...
def test_no_unexplained_differences(fixture):
    """全 21 行 × 42 期：不允许存在未解释差异（BUG=0）"""
    imp, grid, params, salary_07 = fixture
...
def test_match_count_baseline(fixture):
    ...
    assert (categories["MATCH"], categories["KNOWN_QUIRK"],
            categories["KNOWN_RULE_DIFF"], categories["BUG"]) == (347, 80, 60, 0)
...
def test_cash_opening_rolls_like_excel(fixture):
    """期初现金：Excel 静态滚动链与引擎上期末滚动完全一致（42 期）"""
    imp, grid, params, salary_07 = fixture
...
def test_salary_july_from_payroll(fixture):
    """2026-07 工资：Excel 空；引擎按工资表应付合计（元→万元）"""
    imp, grid, params, salary_07 = fixture
    assert salary_07 > 0
    assert imp["excel"]["exp.salary"].get("2026-07") is None
    assert abs(Decimal(grid["exp.salary"]["2026-07"]["value"]) - salary_07) <= TOL
